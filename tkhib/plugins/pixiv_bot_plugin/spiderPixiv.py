...
from tkhib.runtime_paths import get_data_dir
...
class PixivSpider:
    ...
    _sync_file_lock = threading.Lock()
    ...
    def __init__(self, cookie_file: str = "pixiv_cookie.pkl"):
        ...
        self.data_dir = get_data_dir("pixiv")
        self.cookie_file = self._resolve_data_file(cookie_file)
        ...
        # Tag池配置
        self.preferred_tags_file = self._resolve_data_file("preferred_tags.pkl")
        self.blocked_tags_file = self._resolve_data_file("blocked_tags.pkl")
        self.preferred_tags: Set[str] = set()
        self.blocked_tags: Set[str] = set()
        
        # 喜欢作者配置
        self.favorite_authors_file = self._resolve_data_file("favorite_authors.pkl")
    ...
    def _resolve_data_file(self, filename: str) -> Path:
        path = Path(filename)
        if path.is_absolute():
    ...
    def save_cookie(self, cookie_string: str, user_id: Optional[str] = None, keep_existing_id: bool = True) -> bool:
        ...
        try:
            ...
            with PixivSpider._sync_file_lock:
    ...
    async def _should_block_image_with_translation(self, illust_tags: List[str], related_tags: Optional[List[str]] = None, tag_translation: Optional[Dict[str, Dict[str, str]]] = None) -> bool:
        ...
        try:
            ...
            print(f"检查标签屏蔽: 图片标签 {len(illust_tags_lower)} 个，屏蔽标签 {len(blocked_tags_lower)} 个")
                    ...
                    for related_tag in limited_related_tags:
                        ...
                        for blocked_tag in blocked_tags_lower:
                            if related_tag_lower == blocked_tag or blocked_tag in related_tag_lower:
                                print(f"相关标签匹配屏蔽: '{related_tag}' 匹配屏蔽tag '{blocked_tag}'")
                                return True
            ...
            print(f"翻译检查范围: {len(tags_to_check_translation)} 个标签（从 {len(illust_tags) + (len(related_tags) if related_tags else 0)} 个中筛选）")
                ...
                for lang, translated in translations.items():
                    ...
                    for blocked_tag in blocked_tags_lower:
                        ...
                        if translated_lower == blocked_tag:
                            ...
                            print(f"翻译匹配屏蔽: {tag_type} '{tag}' ({lang}: '{translated}') 精确匹配屏蔽tag '{blocked_tag}'")
                            return True
                        ...
                        if len(blocked_tag) >= 2:  # 至少2个字符才考虑模糊匹配
                            ...
                            if re.search(pattern, translated_lower, re.IGNORECASE):
                                tag_type = "图片标签" if tag in illust_tags else "相关标签"
                                print(f"翻译匹配屏蔽: {tag_type} '{tag}' ({lang}: '{translated}') 词汇匹配屏蔽tag '{blocked_tag}'")
                                return True
        ...
        try:
            ...
            if isinstance(tag, list):
                ...
                if len(tags_list) > 1:
                    ...
                    print(f"标签拆解: '{tag_str}' -> 拆分为 {tags_list} -> 合并关键词: '{search_keyword}'")
                else:
                    # 单个标签
                    search_keyword = tags_list[0] if tags_list else tag_str
                    print(f"单个标签: '{tag_str}' -> 搜索关键词: '{search_keyword}'")
    ...
    async def _get_illust_pages_unlimited(self, illust_id: str) -> Optional[List[Dict[str, Any]]]:
        try:
            ...
            if pages_data.get('error'):
                print(f"获取图片页面失败: {pages_data.get('message', '未知错误')}")
                return None
    ...
    def _replace_image_url(self, url: str) -> str:
        ...
        try:
            ...
            # 如果发生了替换，记录日志
            if replaced_url != url:
                print(f"URL替换: {url} -> {replaced_url}")
        ...
        try:
            ...
            if isinstance(url_or_urls, str):
                ...
                for url in url_or_urls:
                    if isinstance(url, str) and url.strip():
                        ...
                        if url_str.strip():
                            ...
                            result_urls.append(replaced_url)
                            print(f"URL类型转换: {type(url)} -> str, 值: {replaced_url}")
            else:
                # 其他类型，尝试转换为字符串
                ...
                if url_str.strip():
                    replaced_url = self._replace_image_url(url_str.strip())
                    result_urls.append(replaced_url)
                    print(f"URL类型转换: {type(url_or_urls)} -> str, 值: {replaced_url}")
    ...
    async def get_illust_original_urls(self, illust_id: str) -> Optional[List[str]]:
        ...
        try:
            ...
            print(f"获取到 {len(original_urls)} 个原始图片URL - ID: {illust_id}")
            return original_urls
            
        except Exception as e:
            print(f"获取原始图片URL异常: {e}")
            return None
    ...
    async def get_tag_latest_images(self, 
        ...
        try:
            ...
            while len(found_images) < count and page <= max_pages:
                ...
                for image in page_images:
                    ...
                    if create_time >= time_threshold:
                        ...
                        print(f"找到有效图片 {len(found_images)}/{count}: ID={image.get('id')}, 上传时间={hours_since_upload:.1f}小时前")
                        
                        # 数量足够，立即返回
                        if len(found_images) >= count:
                            print(f"已找到足够图片，停止搜索")
                            break
                    else:
                        # 如果图片不在时间范围内，且这是按时间排序的，说明后面的图片更旧
                        hours_since_upload = (datetime.now(timezone.utc) - create_time).total_seconds() / 3600
                        print(f"图片 {image.get('id')} 不在时间范围内（{hours_since_upload:.1f}小时前），可能已到时间边界")
                        # 可以选择继续检查几张，或者直接跳到下一页
                
                print(f"第 {page} 页检查完成: 找到 {page_valid_count} 张有效图片，累计 {len(found_images)} 张")
                
                # 如果当前页没有新图片，可能已经到了最早的页面
                if page_valid_count == 0:
                    print(f"第 {page} 页没有符合时间范围的图片，停止搜索")
                    break
            ...
            print(f"总共找到 {len(found_images)} 张最新图片")
    ...
    async def _search_illustrations_simple(self, 
        ...
        try:
            ...
            print(f"简化搜索图片 - 关键词: '{search_keyword}', 页码: {page}")
    ...
    async def _extract_simplified_data_simple(self, 
        ...
        try:
            ...
            # 记录屏蔽统计
            if blocked_count > 0:
                print(f"翻译Tag屏蔽: 过滤了 {blocked_count} 个包含屏蔽tag的作品")
            ...
            print(f"简化搜索完成: 找到 {len(simplified_illusts)} 个结果，总计 {simplified_data['total']} 个")
            return simplified_data
    ...
    async def get_image_info(self, illust_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            print(f"获取图片信息 - ID: {illust_id}")
            
            # 获取图片详细信息
            illust_details = await self.get_illust_details(illust_id)
            if not illust_details:
                print(f"获取图片详情失败: {illust_id}")
                return None
            ...
            print(f"成功获取图片信息 - ID: {illust_id}, 标题: {image_info['title']}, URL数量: {len(urls_field)}, Tags数量: {len(tags_field)}")
            return image_info
    ...
    async def get_author_info(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            user_id = str(user_id).strip()
            print(f"获取作者信息 - ID: {user_id}")
            
            # 获取用户详细信息
            user_info = await self.get_user_info(user_id)
            if not user_info:
                print(f"获取用户信息失败: {user_id}")
                return None
            ...
            print(f"成功获取作者信息 - ID: {user_id}, 名称: {author_info['name']}, 作品数: {total_works}")
            return author_info
